...
function buildServer(directory, config = {}) {
  const typeDefs = globSync(`${directory}/types/**/*.?(graphql|gql)`).map(readTypeFile);
  const require = esm(module);
  const resolvers = requireFile(require, `${directory}/resolvers`);
  const mocks = config.mocks === false ? config.mocks : requireFile(require, `${directory}/mocks`) || config.mocks || false;
